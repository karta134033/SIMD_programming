...
{
  for (int i = 0; i < VECTOR_WIDTH; i++)  // dest.value array's size is predefined in VECTOR_WIDTH
  {                                       // src's size is ((VECTOR_WIDTH = 4) + (N = 16))  
    dest.value[i] = mask.value[i] ? src[i] : dest.value[i];  // if (mask) than use src value, otherwise use dest value
  }
