...
int main(int argc, char *argv[])
{
  ...
  float *values = new float[N + VECTOR_WIDTH];  // VECTOR_WIDTH = 4, N = 16  
...
void initValue(float *values, int *exponents, float *output, float *gold, unsigned int N)
{
  ...
  {
    // random input values
    values[i] = -1.f + 4.f * static_cast<float>(rand()) / RAND_MAX;  // cast rand() to float than do the rest calculation
